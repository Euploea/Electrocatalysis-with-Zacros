...
class SimIn():
    ...
    def WriteIn(self, fldr):
        ...
        with open(os.path.join(fldr, self.fname), 'w') as txt:
            txt.write('#####################################################\n')
            txt.write('# Zacros Simulation Input File  \n')
            txt.write('#  Ilaria Maresi May 2018 and Jinghan Li May 2020 \n')
            txt.write('#  Program modified to simulate electrochemistry\n')
            txt.write('#####################################################\n\n')
            
            # if a random seed does not exist make a new one
            SeedTxt = ''
            ...
            # JWA increased precision of the temperature field
            if self.TPD:
                txt.write('{:20} {:5.4f} {:5.4f}\n'.format('temperature\t ramp',
...
class StateIn(object):
    ...
    def WriteIn(self, fldr, surf_spec):
        ...
        if self.Type is None:
            ...
            with open(os.path.join(fldr, self.fname), 'w') as txt:
                for i in self.Struct:            # self.Structure is a list with all the lines of state_input.dat in it
                    txt.write(i + '\n')          # JWA - this just write the whole file back with no changes
